...
class DES:
    ...
    def __init__(self, des_key: bytes, mac_key: bytes) -> None:
    ...
    def encrypt(self, msg_str: str) -> bytes:
        ...
        # concatenate mac and msg here
        mac = sha256(msg_bytes+ self.mac_key).digest()
        # mac: compute mac here
        #two 
        print("plaintext: " ,msg_str)
        print("HMAC: ", mac.hex())
        msg_bytes += mac
        ...
        print("sent ciphertext: ",cipher_byts.hex())
        # now return both cipher and mac
    ...
    def decrypt(self, msg_bytes: bytes) -> str:
        ...
        """
        plain_bits = []
        print("Recieved ciphertxt: ", msg_bytes.hex())
        l = len(msg_bytes)
        for i in range(len(msg_bytes) // 8):
            block_bytes = msg_bytes[i * 8 : (i + 1) * 8]
        ...
        # now the bytes contains mac as well
        plain_byts_with_mac = debitize(plain_bits)
        # separate them here

        plain_byts = plain_byts_with_mac[:l-32]
        mac = plain_byts_with_mac[l-32:]
        print("Plaintext: ",plain_byts.decode('utf-8'))
        print("HMAC: ",mac.hex())
        #split 
        # num 5 prinln
        # compute and verify mac here
        expected_mac = sha256(plain_byts+ self.mac_key).digest()
        print("calculated Hmac: ",expected_mac.hex())
        if(expected_mac == mac):
            print("Mac's match")
        else:
            print("Mac's don't match")
        # now return the plain text and mac (both decoded)
        # just a placeholder return now
        # you may returen them as a tuple like: plaintext, mac
        return plain_byts.decode('utf-8'),mac.hex()
